...
public class CompraController {
    ...
    @PutMapping("/add-produto/{id}")
    public ResponseEntity<CompraResponse> adicionarProduto(@RequestBody ProdutosResponse produto, @PathVariable Long id) {
        Optional<CompraDTO> compra = service.obterCompra(id);
        CompraDTO compra_dto = compra.get();
        service.adicionarProduto(produto, compra_dto);
        CompraResponse compra_response = new CompraResponse();
        BeanUtils.copyProperties(compra_dto, compra_response);
        return new ResponseEntity<>(compra_response, HttpStatus.ACCEPTED);
    }

    @DeleteMapping("/finalizar-compra")
    public ResponseEntity<CompraResponse> finalizarCompra(@RequestBody CompraRequest compra){
        CompraDTO finalizar_compra = new CompraDTO();
        BeanUtils.copyProperties(compra, finalizar_compra);
        service.finalizarCompra(finalizar_compra);
        CompraResponse compra_response = new CompraResponse();
        BeanUtils.copyProperties(compra, compra_response);
        return new ResponseEntity<>(compra_response, HttpStatus.ACCEPTED);
    }
